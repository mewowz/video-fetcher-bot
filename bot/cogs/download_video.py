...
class DownloadVideoCog(commands.Cog):
    ...
    async def dl(
        ...
        self.logger.debug(
            f"Pushing job to '{NEW_JOBS_QUEUE}' on valkey server "
        ...
        self.logger.info(f"Successfully pushed job {payload['job_id']} to queue. Returning")
    ...
    async def dl_error(self, interaction: discord.Interaction, error: app_commands.AppCommandError):
        ...
        #    self._handle_http_err(error)
            ...
            await interaction.followup.send("Something went wrong enqueuing that request.")
